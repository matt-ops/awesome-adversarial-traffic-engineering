...
- **Canonical lesson:** `docs/appendices/traffic-bot-threat-intelligence/03-intelligence-to-emulation-and-regression.md`
- **Checkpoint:** Optional appendix; excluded from core checkpoint time
- **Prerequisite:** Python 3.12+; no core lesson or appendix lesson is required
...
- **Expected evidence or output:** standard-output intelligence record including contradictions, alternatives, confidence, and exact regression criteria
- **Cleanup:** none; do not commit generated private notes or telemetry
- **Retest use:** repeat after fixture or indicator-version changes and compare lifecycle, cluster support, and regression definitions

### `python -m lab.analysis.classifier_tradeoffs`

- **Canonical lesson:** `docs/modules/05-control-recon/06-classifier-evaluation-and-adversarial-drift.md`
- **Checkpoint:** 21 days / Integrated
- **Prerequisite:** blocked-baseline populations and confusion-matrix arithmetic
- **Offensive objective:** compare two declared thresholds before and after adversarial drift while quantifying base-rate, near-neighbor, operational-cost, and protected-action effects
- **Protected action or service effect:** synthetic stock and adapted abuse completions plus legitimate near-neighbor friction at thresholds `0.50` and `0.75`
- **Expected output:** confusion matrix, precision, recall, false-positive and false-negative rates, flagged volumes, review/challenge/collateral cost, and base-rate example
- **Interpretation:** the high threshold loses adapted abuse after drift while the low threshold preserves recall at substantial legitimate cost; a score change alone is not offensive success
- **Source basis:** `scikit-learn-classification-metrics`, `axelsson-base-rate`, `google-ml-monitoring`, `aate-local-lab`
- **Safety boundary:** deterministic aggregate fixture; no model training, personal data, service, or network request
- **Expected evidence or output:** exact baseline/post-drift operating-point rows and protected-action completion comparison
- **Cleanup:** none; the command writes no output file
- **Retest use:** repeat the same populations, thresholds, cost model, and protected-action fields after a declared remediation

### `python -m lab.protocol.compare automated`

- **Canonical lesson:** `docs/modules/07-protocol-identity/06-local-multi-client-protocol-comparison.md`
- **Checkpoint:** 6 weeks / Deep
- **Prerequisite:** TLS ClientHello fields, HTTP/2 streams/settings, and observation-point limits
- **Offensive objective:** capture real loopback ClientHello and HTTP/2 differences from installed Python/OpenSSL, curl, Playwright Chromium, and Node clients
- **Protected action or service effect:** two harmless HTTP/2 requests on one local session; no authorization decision or external service effect
