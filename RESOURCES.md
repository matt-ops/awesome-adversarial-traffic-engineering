...
Use only the target assigned by the provider or your isolated self-hosted environment. A link is a learning assignment, not permission to test the provider’s ordinary website or any unrelated system. Keep five notes: attack goal, technique, bypass evidence, remediation, and retest. A badge, flag, or completion percentage by itself is not the learning result.
